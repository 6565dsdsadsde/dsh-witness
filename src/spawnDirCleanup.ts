...
function restoreFileAcl(f: string): boolean {
  try {
 ...
 */
export function cleanupSpawnDir(spawnDir: string): boolean {
  try {
    if (!fs.existsSync(spawnDir)) return true
    const lock = path.join(spawnDir, 'lock')
    // ① lock 残留（崩溃收养的 C5 化证据）：R4 恢复
    let lockStatOk = true
    try { fs.statSync(lock) } catch { lockStatOk = false }
    if (lockStatOk) {
      // 先尝试轻量 icacls（正常 deny WD+AD 场景），失败再走 R4（C5 化场景）
      try { spawnSync('icacls', [lock, '/remove:d', 'Everyone'], { windowsHide: true, timeout: 8000 }) } catch {}
      try { spawnSync('attrib', ['-R', lock], { windowsHide: true, timeout: 8000 }) } catch {}
      let stillProtected = false
      try { fs.rmSync(lock, { force: true }) } catch { stillProtected = true }
      if (stillProtected) {
        if (!restoreFileAcl(lock)) return false   // R4 失败：本轮放弃，下轮重试
        try { fs.rmSync(lock, { force: true }) } catch { return false }
      }
    }
    ...
    for (const f of fs.readdirSync(spawnDir)) {
      ...
      try {
        if (fs.statSync(fp).isFile()) {
          try { spawnSync('icacls', [fp, '/remove:d', 'Everyone'], { windowsHide: true, timeout: 8000 }) } catch {}
          try { spawnSync('attrib', ['-R', fp], { windowsHide: true, timeout: 8000 }) } catch {}
          // 验证是否可删（C5 化文件仍删不掉 → R4 恢复）
          try { fs.rmSync(fp, { force: true }) } catch {
            if (!restoreFileAcl(fp)) return false
            try { fs.rmSync(fp, { force: true }) } catch { return false }
          }
        }
      } catch { /* 竞态消失 */ }
    }
    // ③ 递归删除
    try { fs.rmSync(spawnDir, { recursive: true, force: true }) } catch { return false }
    return true
  } catch { return false }
}
