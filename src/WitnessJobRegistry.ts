// src/WitnessJobRegistry.ts —— 单机无协调任务系统（范式深处实现，Day 1 骨架）
// 真相源 = 目录结构（状态 = state/ 目录存在性函数）；协调 = O_EXCL 锁；SQLite = 只读索引缓存
// 依据：状态机 fuzz（25,887 断言）+ 索引缓存实测（3.6x）验证的架构要点
...
const existsOk = (p: string): boolean => {
  try { fs.statSync(p); return true } catch { return false }
}
// 沙箱 v3：deny 生效瞬间对已开句柄/新建打开的瞬时 EPERM 存在竞态窗口——观测读一律重试（瞬时拒绝 ≠ 真相）
const readOk = (p: string): string | undefined => {
  for (let i = 0; i < 5; i++) {
    try { return fs.readFileSync(p, 'utf-8') } catch { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 100) }
  }
  return undefined
}
...
export interface WitnessConfig {
  jobsRoot?: string        // 任务目录根（真相源）
  indexDbPath?: string     // SQLite 索引缓存（账房）
  heartbeatMs?: number
  adoptMonitorMs?: number
}

const STATE_DIRS = ['running', 'stopping', 'orphaned', 'adopted', 'done'] as const

export class WitnessJobRegistry {
  private jobsRoot: string
  private db: DatabaseSync
  private selfCtx: Context
  ...
  constructor(ctx: Context, config: WitnessConfig = {}) {
    ...
    if (monitorMs > 0) this.timers.push(setInterval(() => { try { this.adoptMonitor() } catch {} }, monitorMs))
    ctx.effect(() => () => { this.stopped = true; for (const t of this.timers) clearInterval(t); try { this.db.close() } catch {} }, 'witness teardown')
  }
  ...
  private recover(): void {
    for (const f of fs.readdirSync(this.jobsRoot)) {
      ...
      if (obs.state === 'done' || obs.state === 'absent') continue
    ...
    for (const f of fs.readdirSync(this.jobsRoot)) {
      ...
      if (obs.state === 'done') {
        ...
        try {
          ...
          if (ageMs > 24 * 3600 * 1000) cleanupSpawnDir(d)
        } catch { /* 下轮重试 */ }
        continue
      }
      if (obs.state === 'running' || obs.state === 'adopted' || obs.state === 'orphaned' || obs.state === 'stopping') {
        if (!this.lockAlive(d)) this.finalizeFromExit(f, d)
      }
    }
  }
  ...
  private finalize(id: string, d: string, explicitCode?: number): void {
    try {
    if (existsOk(path.join(d, 'state', 'done'))) return   // 终态已由并发胜者落盘
    ...
    let totalBytes = 0
    if (existsOk(outFile)) { try { totalBytes = fs.statSync(outFile).size } catch {} }
    if (totalBytes > 0) this.event(id, 'output', { totalBytes })
    // 尸检报告（验收 C-02：manner_of_death + primary_evidence + verdict + 死因代码 D-01~D-09）
    const evidence: string[] = []
    if (existsOk(path.join(d, 'lock'))) evidence.push('lock')
    if (existsOk(exitFile)) evidence.push('exit.txt')
    if (existsOk(outFile)) evidence.push('out.log')
    if (existsOk(path.join(d, 'state', 'running'))) evidence.push('state/running')
    const wasStopping = existsOk(path.join(d, 'state', 'stopping'))
    let deathCode: string
    let manner: string
    if (code === -999) { deathCode = 'D-06'; manner = 'evidence tampered by task (lock content/ACL altered)' }
    else if (wasStopping) { deathCode = 'D-03'; manner = 'killed via stop request' }
    else if (code === 0) { deathCode = 'D-01'; manner = 'completed normally' }
    else if (explicitCode === undefined && !existsOk(exitFile)) { deathCode = 'D-08'; manner = 'adopted with no exit file (crash before exit write)' }
    else { deathCode = 'D-02'; manner = 'exited non-zero' }
  ...
  private finalizeFromExit(id: string, d: string): void { this.finalize(id, d) }

  /** 状态标记转移：tmp + rename 原子 + 显式 touch state/ 目录（评审 4.4：防 mtime 精度竞态） */
  private markState(id: string, state: (typeof STATE_DIRS)[number]): void {
    const sd = path.join(this.taskDir(id), 'state')
    // 清旧标记（done 除外——done 保留 exit 码语义）
    for (const s of STATE_DIRS) {
      if (s === state || s === 'done') continue
      try { fs.unlinkSync(path.join(sd, s)) } catch { /* 不存在 */ }
    }
    const tmp = path.join(sd, `.tmp-${id}-${state}`)
    fs.writeFileSync(tmp, '')
    try { fs.renameSync(tmp, path.join(sd, state)) } catch { try { fs.unlinkSync(tmp) } catch {} }   // 并发 rename 竞争：败者静默
  ...
  read(id: string): string {
    ...
    const row = this.db.prepare('SELECT out_cursor FROM job_index WHERE id=?').get(id) as { out_cursor: number } | undefined
  ...
  private nextId(kind: string): string {
    const row = this.db.prepare(`INSERT INTO job_seq (kind, count) VALUES (?,1) ON CONFLICT(kind) DO UPDATE SET count=count+1 RETURNING count`).get(kind) as { count: number }
    return `${kind}-${row.count}`
  }

  // ---------------- 证人：状态 = 目录结构函数 ----------------
  private taskDir(id: string) { return path.join(this.jobsRoot, id) }

  /** 目录观察（真相读）：state/ 目录存在性 + lock 存活 */
  private observe(id: string): { state: WitnessState; exitCode?: number } {
    const d = this.taskDir(id)
    if (!existsOk(d)) return { state: 'absent' }
    const done = path.join(d, 'state', 'done')
    if (existsOk(done)) {
      try { return { state: 'done', exitCode: Number(fs.readFileSync(done, 'utf-8').trim()) || undefined } } catch { return { state: 'done' } }
    }
    const has = (s: string) => existsOk(path.join(d, 'state', s))
    if (has('running')) return { state: this.lockAlive(d) ? 'running' : 'orphaned' }
    if (has('stopping')) return { state: this.lockAlive(d) ? 'stopping' : 'orphaned' }
    if (has('adopted')) return { state: this.lockAlive(d) ? 'adopted' : 'orphaned' }
    if (has('orphaned')) return { state: 'orphaned' }
    return { state: 'orphaned' }   // 有目录无标记 = 孤儿（崩溃在标记创建前）
  }

  /** lock 存活判定（O_EXCL 锁内容 pid:startSec + 进程存在 + 启动时间比对） */
  private lockAlive(d: string): boolean {
    const lock = path.join(d, 'lock')
    if (!existsOk(lock)) return false
    try {
  ...
  private procStartSec(pid: number): number | undefined {
    try {
  ...
  private dirSignal(id: string): number {
    const d = this.taskDir(id)
  ...
  get(id: string): JobSnapshot {
    ...
    const cached = this.db.prepare('SELECT cached_state, dir_mtime_ms FROM job_index WHERE id=?').get(id) as { cached_state: string; dir_mtime_ms: number } | undefined
    let state: WitnessState
    let exitCode: number | undefined
    if (cached !== undefined && cached.dir_mtime_ms === dirMtime) {
      state = cached.cached_state as WitnessState   // 缓存命中
    } else {
  ...
  private snapshotOf(id: string, state: WitnessState, exitCode?: number): JobSnapshot {
    const specPath = path.join(this.taskDir(id), 'spec.json')
    let kind = 'unknown', label = '', startedAt = 0
    try {
      const spec = JSON.parse(fs.readFileSync(specPath, 'utf-8'))
      kind = spec.kind; label = spec.label; startedAt = spec.startedAt
    } catch { /* spec 缺失：目录已存在但规格丢失（罕见） */ }
    if (state === 'done' && exitCode === undefined) {
      // 缓存命中路径：done 的退出码直接读真相文件（不依赖传参）
      try { exitCode = Number(fs.readFileSync(path.join(this.taskDir(id), 'state', 'done'), 'utf-8').trim()) || 0 } catch { exitCode = 1 }
    }
    // tampered 协议（沙箱 v5 留痕检测）：exitCode=-999 → 状态显式 tampered（快照可见，非笼统 failed）
    const status = state === 'done' ? (exitCode === -999 ? 'tampered' : exitCode === 0 ? 'completed' : 'failed') : state === 'absent' ? 'failed' : state as string
    return { id: id as never, kind: kind as never, label, status: status as JobSnapshot['status'], startedAt, reported: false } as JobSnapshot
  }

  // ---------------- start：目录 + O_EXCL 锁 + detached 子进程（Day 1 检查点） ----------------
  start(spec: JobStart & { command?: string }): string {
    ...
    } else if ((spec as unknown as { run?: () => { done: Promise<any> } }).run !== undefined) {
      // 对齐官方：run() hooks 提供 done——手动终态路径（无 command 的状态机任务）
      const hooks = (spec as unknown as { run: () => { done: Promise<any> } }).run()
      hooks.done.then((outcome) => {
        const code = (outcome as any).exitCode ?? (outcome.status === 'completed' ? 0 : 1)
  ...
  list(): JobSnapshot[] {
    ...
    for (const f of fs.readdirSync(this.jobsRoot)) {
      if (fs.statSync(path.join(this.jobsRoot, f)).isDirectory()) out.push(this.get(f))
    }
    return out
  }

  /** wait：轮询至终态（done/tampered）或超时——验收 A-01/A-02/B-02 的等待语义 */
  async wait(id: string, timeoutMs: number): Promise<JobSnapshot> {
    const deadline = Date.now() + timeoutMs
    for (;;) {
      const snap = this.get(id)
      const st = snap.status as string
      if (st === 'completed' || st === 'failed' || st === 'tampered') return snap
      if (Date.now() > deadline) return snap   // 超时返回当前快照（调用方断言状态）
  ...
  kill(id: string): 'requested' | 'already-finished' {
    ...
    if (obs.state === 'done' || obs.state === 'absent') return 'already-finished'
    const lock = path.join(this.taskDir(id), 'lock')
    if (existsOk(lock)) {
      try {
        const m = /^(\d+):(\d+)$/.exec(fs.readFileSync(lock, 'utf-8').trim())
        if (m !== null) process.kill(Number(m[1]))
      } catch { /* 已死 */ }
    }
  ...
  private event(id: string, change: string, payload: unknown): void {
    try {
...
export function apply(ctx: Context, config: WitnessConfig = {}) {
